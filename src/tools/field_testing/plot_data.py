"""plot_data.py  –  Field-testing session analysis
====================================================
Reads the CSV files written by receiver.py and produces one figure per remote
node plus one for the gateway.
...
Usage:
    python src/tools/field_testing/plot_data.py                 # reads <repo>/field-testing-data
    python src/tools/field_testing/plot_data.py --data-dir field-testing-data/run2
"""
...
# Default CSV directory: <repo>/field-testing-data (matches receiver.py).
_DEFAULT_DATA_DIR = Path(__file__).resolve().parents[3] / "field-testing-data"
...
def main():
    parser = argparse.ArgumentParser(description="Plot LoRa TestBed field-testing session data")
    parser.add_argument(
        "--data-dir", default=str(_DEFAULT_DATA_DIR),
        help="Directory containing CSV files (default: <repo>/field-testing-data)",
    )
